...
- [`action-plan-ir.md`](./action-plan-ir.md)：边缘唯一执行契约、IR 对象模型、步骤语义、结果模型与校验规则
- [`action-plan-schema.md`](./action-plan-schema.md)：`ActionPlan v1alpha1` 的字段级 schema、步骤类型与约束定义
- [`action-result-schema.md`](./action-result-schema.md)：`ActionResult v1alpha1` 和 `StepActionRecord` 的字段级 schema
- [`agentd-architecture.md`](./agentd-architecture.md)：`wp-agentd` 模块拆分、本地状态机、调度职责与进程边界
- [`agentd-exec-protocol.md`](./agentd-exec-protocol.md)：`wp-agentd` 与 `wp-agent-exec` 的本地协议、工作目录、状态文件与取消机制
- [`agentd-state-and-boundaries.md`](./agentd-state-and-boundaries.md)：`wp-agentd` 的本地状态分层、唯一写入权、状态机拥有者与模块协作边界
- [`agentd-state-schema.md`](./agentd-state-schema.md)：`wp-agentd` 本地状态文件的字段级 schema 草案
- [`agentd-events.md`](./agentd-events.md)：`wp-agentd` 模块协作事件对象、最小事件流与审计事件边界
- [`capability-report-schema.md`](./capability-report-schema.md)：agent 能力声明、能力匹配和限制字段的 schema 草案
- [`agent-config-schema.md`](./agent-config-schema.md)：`wp-agentd` 本地总配置骨架和各配置段结构
- [`error-codes.md`](./error-codes.md)：统一错误码、原因码和使用建议
- [`self-observability.md`](./self-observability.md)：`wp-agent` 自身的 metrics、logs、events 与验收观测面设计
- [`metrics-integration-roadmap.md`](./metrics-integration-roadmap.md)：metrics integration 覆盖范围、优先级、分批落地与 exporter 策略
- [`metrics-batch-a-plan.md`](./metrics-batch-a-plan.md)：Batch A 的 target 范围、最小指标集、resource 绑定与验收标准
- [`metrics-config-schema.md`](./metrics-config-schema.md)：metrics integration 的统一配置骨架、通用字段与 budget 结构
- [`metrics-batch-a-specs.md`](./metrics-batch-a-specs.md)：`host_metrics`、`process_metrics`、`prom_scrape`、`otlp_metrics_receiver` 的规格草案
- [`metrics-discovery-and-resource-mapping.md`](./metrics-discovery-and-resource-mapping.md)：metrics target discovery、去重、resource binding 与高基数控制规则
- [`action-dsl.md`](./action-dsl.md)：中心侧动作 DSL、边缘执行 IR、opcode 白名单和审批绑定
- [`control-plane.md`](./control-plane.md)：控制对象、生命周期状态机、中心到边缘协议与编排闭环
- [`roadmap.md`](./roadmap.md)：多里程碑开发路线图、阶段目标、验收标准与依赖关系
- [`run-gxl-subset.md`](./run-gxl-subset.md)：`run.gxl` 的受限语法子集、AST、opcode 映射与编译边界
- [`run-gxl-construct-mapping.md`](./run-gxl-construct-mapping.md)：`step/when/expect/emit/fail/retry` 与当前 GXL 的映射和实现策略
- [`action-schema.md`](./action-schema.md)：各 opcode 的参数 schema、返回 schema、风险、限制与 capability
...
- 实现最小可运行的自研环境内采集 pipeline
- 支持主机与 Kubernetes 两种部署形态
- 支持 OTLP 作为优先标准输入 / 输出之一
- 建立原始事件封装
- 建立基础资源模型
- 自动发现 host / process / container / pod / service
- 输出 logs 与 security 两类结果
- 建立 CPU / 内存占用的基础观测与压测基线
- 建立最小可用的升级协议与升级状态回报机制
...
10. 第一阶段必须先打通“安装 -> 发现 -> 采集 -> 上送 -> 中心关联”的最小闭环。
11. 长期目标是形成更适合目标环境、且尽量符合 OpenTelemetry 标准的一体化环境观测系统。
