...
- 环境内 `warp-insightd`、`wist-exec`、`wist-upgrader` 之间的信任边界如何划分
...
- 分层信任：
  中心节点、`warp-insightd`、`wist-exec`、`wist-upgrader` 之间不是同级全信任
- standalone 先收敛：
...
- 边界 A：
  中心节点与环境内 agent 之间
- 边界 B：
  `warp-insightd` 与 `wist-exec` 之间
- 边界 C：
  `warp-insightd` 与 `wist-upgrader` 之间
- 边界 D：
...
- `wist-exec` 是高风险受控执行器，不应默认继承 `warp-insightd` 的全部能力
- `wist-upgrader` 是高风险切换器，不应承担采集和远程动作职责
...
- `wist-exec` 不直接长期持有中心通信主身份
- `wist-upgrader` 不直接长期持有中心通信主身份

建议模型是：

- `warp-insightd` 作为本地根协调者，接收中心指令
- `warp-insightd` 为每次执行动作生成一次性本地执行上下文
- `wist-exec` / `wist-upgrader` 只在该上下文内拥有临时执行权限
...
- 节点执行权限：
  哪些节点允许执行哪些动作
- 本地进程权限：
  `warp-insightd`、`wist-exec`、`wist-upgrader` 各自拥有哪些 OS / runtime 权限
...
- `wist-exec` 只在动作执行期间获得必要权限
- `wist-upgrader` 只在升级窗口内获得安装、切换、回滚所需权限
...
3. `warp-insightd` 拉起 `wist-exec`
4. `wist-exec` 在受限上下文中执行动作
5. `wist-exec` 输出 `ActionResult` 并退出
...
4. `warp-insightd` 拉起 `wist-upgrader`
5. `wist-upgrader` 下载、校验、切换、探活、必要时回滚
...
- `warp-insightd / wist-exec / wist-upgrader` 三进程角色固定
...
- `wist-exec` 和 `wist-upgrader` 是受临时授权驱动的高风险执行进程，不应长期继承主身份和高权限
