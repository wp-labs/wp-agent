# warp-insight 动作 DSL 与执行 IR 设计

## 1. 文档目的

本文档定义 `warp-insight` 的远程动作描述体系，重点回答以下问题：
...
`warp-insight` 需要作者侧描述语言，但当前不应先把 frontend 定死为 `run.gxl` 或 `run.war`。
...
如果让 `warp-insight-exec` 直接解释一个功能很强的 DSL，会带来以下风险：
...
```txt
step p = process.stat(name = "warp-insightd")
expect p.pid > 0
emit p
```
...
`warp-insight` 应该做的是“中心强表达 DSL + 边缘受控执行 IR”，而不是“把一门强 DSL 直接下沉到边缘节点执行”。
