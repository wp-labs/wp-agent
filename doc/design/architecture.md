...
- [`action-plan-schema.md`](./action-plan-schema.md)：`ActionPlan` 的字段级协议定义
- [`action-result-schema.md`](./action-result-schema.md)：`ActionResult` 和 `StepActionRecord` 的字段级协议定义
- [`agentd-architecture.md`](./agentd-architecture.md)：`wp-agentd` 的模块边界、本地状态机和调度职责
- [`agentd-exec-protocol.md`](./agentd-exec-protocol.md)：`wp-agentd` 与 `wp-agent-exec` 的本地交互协议
- [`agentd-state-and-boundaries.md`](./agentd-state-and-boundaries.md)：`wp-agentd` 的本地状态模型、唯一写入权和模块协作边界
- [`agentd-state-schema.md`](./agentd-state-schema.md)：`wp-agentd` 本地状态对象的字段级 schema
- [`agentd-events.md`](./agentd-events.md)：`wp-agentd` 进程内事件对象和模块事件流
- [`capability-report-schema.md`](./capability-report-schema.md)：agent 能力声明和匹配规则
- [`agent-config-schema.md`](./agent-config-schema.md)：`wp-agentd` 总配置骨架
- [`error-codes.md`](./error-codes.md)：统一错误码和原因码词典
- [`self-observability.md`](./self-observability.md)：`wp-agent` 自身可观测性和验收指标面设计
- [`non-functional-targets.md`](./non-functional-targets.md)：资源预算、退化阈值、buffer/backpressure 和保底目标
- [`metrics-integration-roadmap.md`](./metrics-integration-roadmap.md)：metrics integration 的目标分层、优先级和批次规划
- [`metrics-batch-a-plan.md`](./metrics-batch-a-plan.md)：Batch A 的最小 target 覆盖、指标范围和统一配置骨架
- [`metrics-config-schema.md`](./metrics-config-schema.md)：metrics integration 的统一配置结构和公共字段
- [`metrics-batch-a-specs.md`](./metrics-batch-a-specs.md)：Batch A 核心 integration 的规格草案
- [`container-metrics-spec.md`](./container-metrics-spec.md)：`container_metrics` 的字段、发现和资源绑定规格
- [`k8s-node-pod-metrics-spec.md`](./k8s-node-pod-metrics-spec.md)：`k8s_node_pod_metrics` 的字段、发现和资源绑定规格
- [`metrics-discovery-and-resource-mapping.md`](./metrics-discovery-and-resource-mapping.md)：metrics discovery 和 resource mapping 的公共规则
- [`action-dsl.md`](./action-dsl.md)：动作作者 DSL、执行 IR、opcode 白名单和编译边界
- [`control-plane.md`](./control-plane.md)：控制对象、状态机、下发协议与编排闭环
- [`dispatch-action-plan-schema.md`](./dispatch-action-plan-schema.md)：中心向边缘投递 `ActionPlan` 的 envelope schema
- [`ack-action-plan-schema.md`](./ack-action-plan-schema.md)：边缘对计划投递的接收确认 schema
- [`report-action-result-schema.md`](./report-action-result-schema.md)：边缘向中心回报最终结果的 envelope schema
