# warp-agentd 故障处理与恢复设计

## 1. 文档目的

本文档定义 `warp-agentd` 在第一版里的故障分层、恢复语义和健康口径，重点回答三类问题：
...
`warp-agentd` 第一版必须明确以下 source-of-truth 关系。
...
`warp-agentd` 的 runtime health 不能只看 queue 和 running。
