# warp-agentd 与 wist-exec 本地协议设计

## 1. 文档目的

本文档定义 `warp-agentd` 与 `wist-exec` 之间的本地交互协议。

它主要解决以下问题：

- `warp-agentd` 如何把 `ActionPlan` 交给 `wist-exec`
- `wist-exec` 如何回传状态与结果
- 取消、超时、异常退出如何处理
...
- `warp-agentd` 解析 `wist-exec` 的 stdout/stderr 作为正式协议
- 自定义复杂本地 RPC
- 共享内存
...
- `warp-agentd` 负责创建执行工作目录
- `warp-agentd` 把输入文件写入工作目录
- `warp-agentd` 启动 `wist-exec`
- `wist-exec` 读取工作目录中的输入文件
- `wist-exec` 原子写入状态文件和结果文件
- `warp-agentd` 用进程信号驱动取消和强制终止
...
- 结果级摘要与签名由 `warp-agentd` 在读取 `result.json` 后生成
...
`warp-agentd` 应只在检测到 `result.json` 完整落盘后，才认为结果可消费。
...
- `constraints.max_total_duration_ms` 由 `warp-agentd` 监控
...
1. `warp-agentd` 标记本地执行为 `cancel_requested`
2. `warp-agentd` 向 `wist-exec` 发送 `SIGTERM`
3. `wist-exec` 把 `state.json` 更新为 `cancelling`
4. `wist-exec` 做有限清理并写最终 `result.json`
5. 若宽限时间后仍未退出，`warp-agentd` 发送 `SIGKILL`

### 8.3 强制终止

以下情况允许 `warp-agentd` 强制 kill：
...
- `warp-agentd` 与 `wist-exec` 的 v1 正式协议不是 stdout/stderr
