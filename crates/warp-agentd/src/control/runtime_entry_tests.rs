...
fn temp_dir(name: &str) -> PathBuf {
    let suffix = SystemTime::now()
        ...
        .expect("duration")
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("warp-agentd-lib-{name}-{suffix}"));
...
fn resolve_exec_bin_uses_env_override_when_present() {
    ...
    let current_exe = root.join("bin").join("warp-agentd");
...
fn run_from_args_init_config_creates_config_without_exec_bin() {
    ...
    assert!(root.join("warp-agentd").join("agentd.toml").exists());
}
...
fn init_config_message_mentions_config_directory_when_created() {
    let path = Path::new("/tmp/project/warp-agentd/agentd.toml");

    let message = init_config_message(path, true);

    assert!(message.contains("initialized config directory"));
    assert!(message.contains("/tmp/project/warp-agentd"));
    assert!(message.contains("/tmp/project/warp-agentd/agentd.toml"));
}
...
fn usage_message_lists_supported_commands() {
    ...
    assert!(message.contains("warp-agentd init-config [--stdout]"));
    assert!(message.contains("warp-agentd help"));
...
fn resolve_config_root_prefers_visible_directory() {
    ...
    fs::create_dir_all(root.join("warp-agentd")).expect("create visible config dir");
    fs::create_dir_all(root.join(".warp-agentd")).expect("create legacy config dir");
    fs::write(
        root.join("warp-agentd").join("agentd.toml"),
        "schema_version = \"v1\"\n",
    )
    ...
    assert_eq!(resolve_config_root(&root), root.join("warp-agentd"));
}
...
fn resolve_config_root_accepts_legacy_agent_toml_in_visible_directory() {
    ...
    fs::create_dir_all(root.join("warp-agentd")).expect("create visible config dir");
    fs::write(
        root.join("warp-agentd").join("agent.toml"),
        "schema_version = \"v1\"\n",
    )
...
fn resolve_config_root_accepts_legacy_agent_toml_in_hidden_directory() {
    ...
    fs::create_dir_all(root.join(".warp-agentd")).expect("create hidden config dir");
    fs::write(
        root.join(".warp-agentd").join("agent.toml"),
        "schema_version = \"v1\"\n",
    )
