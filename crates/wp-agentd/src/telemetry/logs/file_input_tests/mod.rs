...
fn temp_dir(name: &str) -> PathBuf {
    let suffix = SystemTime::now()
        ...
        .expect("duration")
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("wp-agentd-file-input-{name}-{suffix}"));
...
#[derive(Default)]
struct TestSink {
    records: Vec<wp_agent_contracts::telemetry_record::TelemetryRecordContract>,
    fail_writes: bool,
}
...
fn read_output_records(
    path: &Path,
) -> Vec<wp_agent_contracts::telemetry_record::TelemetryRecordContract> {
