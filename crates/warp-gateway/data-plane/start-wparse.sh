...
#
# 使用 crates/warp-gateway/bin 下自带的 wparse 二进制（版本与 data-plane 工程配套）。
# 用法：
...
#
# 可覆盖环境变量：
#   WPARSE_BIN         wparse 可执行文件路径（默认 crates/warp-gateway/bin/wparse）
...
if [[ ! -x "${WPARSE}" ]]; then
  ...
  echo "expect one at crates/warp-gateway/bin/wparse" >&2
  exit 1
fi
if [[ ! -f "${CONF_FILE}" ]]; then
