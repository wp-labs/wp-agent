...
pub fn run() -> Result<(), String> {
    ...
    eprintln!("warp-insight-exec finished in {}", workdir.root.display());
    Ok(())
}
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let suffix = SystemTime::now()
            ...
            .expect("duration")
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("warp-insight-exec-{name}-{suffix}"));
    ...
    fn write_fixture_workdir(dir: &std::path::Path) {
        ...
        warp_insight_shared::fs::write_json_atomic(&workdir.plan_path, &plan).expect("write plan");
        warp_insight_shared::fs::write_json_atomic(&workdir.runtime_path, &runtime)
    ...
    fn execute_workdir_writes_result_and_state() {
        ...
        let stored_result: warp_insight_contracts::action_result::ActionResultContract =
